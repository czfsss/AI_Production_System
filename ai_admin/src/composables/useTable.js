...
function useTableImpl(config) {
    const { core: { apiFn, apiParams = {}, excludeParams = [], immediate = true, columnsFactory, paginationKey = { current: 'current', size: 'size' } }, transform: { dataTransformer, responseAdapter = defaultResponseAdapter } = {}, performance: { enableCache = false, cacheTime = 5 * 60 * 1000, debounceTime = 300, maxCacheSize = 50 } = {}, hooks: { onSuccess, onError, onCacheHit, resetFormCallback } = {}, debug: { enableLog = false } = {} } = config;
    // 分页字段名配置
