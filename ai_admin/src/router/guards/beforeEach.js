 ...
 * 处理登录状态
 */
async function handleLoginStatus(to, userStore, next) {
 ...
 * 处理动态路由注册
 */
async function handleDynamicRoutes(to, from, next, router) {
    try {
...
function filterEmptyMenus(menuList) {
    return menuList
        ...
        const isEmptyComponentMenu = item.component === '' &&
 ...
 * 根据角色过滤菜单
 */
const filterMenuByRoles = (menu, roles, department) => {
    return menu.reduce((acc, item) => {
        const itemRoles = item.meta?.roles;
        const itemDepts = item.meta?.departments;
